...
public class BrokerJwtValidator implements JwtValidator {
    ...
    public void configure(Map<String, ?> configs, String saslMechanism, List<AppConfigurationEntry> jaasConfigEntries) {
        ...
        if (!expectedAudiences.isEmpty())
            ...
            // Reached only when the audience opt-out is enabled. jose4j otherwise rejects any token that carries an
            // "aud" claim when no expected audience is set, so skip its default audience validation entirely.
